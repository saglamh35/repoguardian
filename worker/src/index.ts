...
// ---- Queue (BullMQ v5: QueueScheduler yok, defaultJobOptions kullan)
const queueName = "dummy";
const attempts = 5;
const backoff: JobsOptions["backoff"] = { type: "exponential", delay: 2000 };

const dummyQueue = new Queue(queueName, {
  connection,
...
app.get("/health", async (_req, res) => {
  try { await connection.ping(); res.json({ ok: true, redis: "ok", queues: { [queueName]: "ready" } }); }
