...
const worker = new Worker(queueName, async (job) => {
  switch (job.name) {
    ...
    case "scan:secrets": {
      const { userId, repoId, repoGithubId } = job.data as { userId: string; repoId: string; repoGithubId: number };
...
// Varsayılan process exit'te Prisma kapat
